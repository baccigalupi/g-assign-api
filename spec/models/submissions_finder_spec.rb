...
describe SubmissionsFinder do
  ...
  let!(:submission_1) { Submission.create(github_name: 'jfoley', assignment_id: 33, submitted_at: Time.now - 3.days) }
  let!(:submission_2) { Submission.create(github_name: 'marthaberner', assignment_id: 22, submitted_at: Time.now - 1.day) }
  ...
  context 'when trying to limit since a certain date' do
    ...
    it "returns only the submissions since that date" do
      expect(finder.results).not_to include(submission_1)
      expect(finder.results).to includesubmission_2)
    end
  end
end
